<template>
    <h1>LUGARES</h1>
    <!-- Buscador -->
    <input type="text" v-model="searchQuery" placeholder="Buscar lugar..." class="search-bar" />
    ...
    <ul>
        <li v-for="location in filteredLocations" :key="location.id">
            <router-link :to="{ name: 'lugar', params: { id: location.id } }">
                <div>
                    <h3>{{ location.name }}</h3>
                    <img :src="location.img === 'unknown' ? '/img/Placeholder.png' : location.img"
                        :alt="location.name" />
                </div>
            </router-link>
        </li>
    </ul>
    <button v-if="currentPage < totalPages" @click="loadMoreLocations">
        Cargar más
    </button>
</template>
...
// Cargar lugares desde la API usando Axios
const fetchLocations = async (page = 1) => {
    try {
        const response = await axios.get("https://api.attackontitanapi.com/locations", {
            params: { page },
        });
